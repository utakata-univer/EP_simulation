...
#pragma _CRI taskcommon n_gathers
    ...
    /* for T3D, finding number of nodes is hard, so only do it once */
#pragma _CRI taskcommon pvm_numnodes
    int pvm_numnodes;

    /* For shared memory, need workspace and synchronization buffers */
    /**long * s_sh_buf;**/
#define CACHESIZE 32  /* size of cache line in bytes */
#define SYNC_SIZE \
(((_SHMEM_BARRIER_SYNC_SIZE*sizeof(long)+CACHESIZE-1)/CACHESIZE)*(CACHESIZE/sizeof(long)))
#pragma _CRI cache_align  s_tmp_buf0,s_tmp_buf1,d_tmp_buf0,d_tmp_buf1
    long s_tmp_buf0[_SHMEM_BARRIER_SYNC_SIZE+1];
    long s_tmp_buf1[_SHMEM_BARRIER_SYNC_SIZE+1];
    long * s_sh_buf[2];
    double d_tmp_buf0[ _SHMEM_REDUCE_MIN_WRKDATA_SIZE ];
    double d_tmp_buf1[ _SHMEM_REDUCE_MIN_WRKDATA_SIZE ];
    double * d_sh_buf[2];
...
/**if(mynode()==0){printf("BLOCKING RECEIVES\n"); fflush(stdout);}**/

    /* Figure out how many nodes we have */
    /* try to send message to node n+1, highest node should get error */
#ifdef NONSENSE
    pvm_initsend( PvmDataRaw );
printf("Node %d initsend\n",mynode()); fflush(stdout);
    pvm_pkint( &pvm_numnodes,1,1);	/* dummy address */
    code = pvm_send( mynode()+1, SEND_INTEGER_TYPE);
    /* receive the message to flush from system */
    if( mynode() != 0 )pvm_recv( ANY_NODE, SEND_INTEGER_TYPE);

    /**if( code==PvmNoTask || code==PvmDupHost ){**/ /* I'm highest node, probably */
    if( code != 0 ){ /* I'm highest node, probably */
printf("Node %d send code = %d\n",mynode(),code); fflush(stdout);
	pvm_numnodes = mynode()+1;
        pvm_initsend( PvmDataRaw );
        pvm_pkint( &pvm_numnodes,1,1);	/* now contains truth */
        pvm_bcast( NULL, SEND_INTEGER_TYPE);
    }
    else{			/* find out number of nodes */
    	pvm_recv( ANY_NODE, SEND_INTEGER_TYPE);
	pvm_upkint( &pvm_numnodes,1,1 );
    }
#endif
/**TEMPORARY**/pvm_numnodes=1;
	envpt = (char *)getenv("MPP_NPES");
	if( envpt==NULL ){
	    printf("Environment variable MPP_NPES not set\n");
	}
	else sscanf( envpt,"%d",&pvm_numnodes);
...
void g_sync(){
  ...
  /**if(this_node==0){
    for(i=1;i<pvm_number_nodes;i++){
      pvm_recv( i, SYNC_TYPE );
    }
    pvm_initsend( PvmDataRaw );
    for(i=1;i<pvm_number_nodes;i++){
      pvm_send( i, SYNC_TYPE );
    }
  }
  else{  ** generic node != 0 **
      pvm_initsend( PvmDataRaw );
      pvm_send( 0, SYNC_TYPE );
      pvm_recv( 0, SYNC_TYPE );
  }**/
}
...
old_doublesum( dpt );
if( fabs(t-*dpt) > 1e-10*fabs(*dpt) ){
printf("DISCREPANCY: node %d, old = %e, new = %e, diff = %e\n",
this_node,*dpt,t,*dpt-t);
fflush(stdout); exit(-1);
}
    *dpt = t;
    /**shmem_barrier(0,0,numnodes(),s_sh_buf[1]);**/
}
