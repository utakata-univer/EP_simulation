...
/*
   ...
   Intel simulator version
      The simulator gets confused by complicated asynchronous message
      passing, so this version uses synchronous sends and receives.
...
int n_send_msgs, n_recv_msgs;
	...
	/* start the send */
	mbuf[i].msg_id = 99;
	csend( GATHER_MSG_TYPE(field,mynode(),index),
	...
	/* For simulator, garble the pointer to catch people who use it before
	   they wait */
	mbuf[i].msg_buf--;
    }
...
int n_send_msgs, n_recv_msgs;
	...
	mbuf[i].msg_id = 99;
	csend( GATHER_MSG_TYPE(field,mynode(),index),
	    mbuf[i].msg_buf, mbuf[i].msg_size, compt->othernode, NODE_PID );
    }
    /* for each node which has neighbors of my sites */
...
register int i;
    if(mbuf==NULL)return;
    /* For simulator, just ungarble the pointers */
    /* messages were synchronous, so don't wait */
    for(i=0; mbuf[i].msg_buf !=NULL; i++)mbuf[i].msg_buf++;
    return;
}
...
int type;		/* message type - nread needs its address */
	...
	mbuf[i+n_recv_msgs+1].msg_id = 99;
	csend( GENERAL_GATHER_MSG_TYPE(this_node),
	...
	/* post receive */
	mbuf[i].msg_id = 199;
	crecv( GENERAL_GATHER_MSG_TYPE(from_nodes[i].node),
...
register int j;
    if(size%4 != 0)printf("WARNING: noninteger memcpy\n");
...
static char name[]="Intel simulator";
char * machine_type(){
    return(name);
}
...
/**
double work;
    gdsum( &(cpt->real),1,&work);
    gdsum( &(cpt->imag),1,&work);
**/
double_complex work;
    gdsum( cpt,2,&work);
}
...
/* Intel simulator doesn't support dclock() at the moment */
double dclock(){
    return( ((double)mclock())/1000.0 );
}
