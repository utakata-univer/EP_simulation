...
int n_send_msgs, n_recv_msgs;
	...
	pvm_send( pvm_node_tid[mbuf[i].msg_node],
	    GATHER_MSG_TYPE(field,mynode(),index) );
    }
    /* terminate list */
    if(mbuf != NULL){
...
send_parameters(buf) params *buf; {
  ...
  for(i=0; i < pvm_number_nodes; i++) if(i != mynode())
    pvm_send( pvm_node_tid[i], PARAM_TYPE);
}
...
void g_sync(){
  int othernode;
  /* Each node sends SYNC_TYPE message to node n+1, waits to get
     its own message */

  othernode = my_logical_node+1;
  if( othernode==pvm_number_nodes )othernode=0;
  pvm_initsend( PvmDataRaw );
  pvm_send( pvm_node_tid[othernode], SYNC_TYPE );
  pvm_recv( ANY_NODE,  SYNC_TYPE );
  ...
  if(mynode()==0)
    {
      ...
      pvm_pkfloat( fpt, 1,1 );
      for(i = 0; i < pvm_number_nodes; i++) if(i != mynode())
	pvm_send( pvm_node_tid[i], BROADCAST_FLOAT_TYPE);
    }
