...
/* com_cmmd3.c:  Interim version.  For CMMD version3, but still use
   cooperative message passing.  11/12/92  DT
*/

/*
   This file is machine dependent.
   CM5 version 1,  1/25/92 DT
   ...
   to gather neighbors.  On the nCUBE machine the sends and receives
   are actually synchronous.
   wait_gather()  waits for receives to finish, insuring that the
   data has actually arrived.  On the nCUBE machine, wait_gather reall
   does nothing.
...
#define GATHER_MSG_TYPE(offset,dir) \
   (GATHER_BASE + ((offset)<<mt_offshift) + (dir))
...
/* TEMPORARY.  Until CM5 gets asynchronous communications, keep track
of which gathers can use CMMD_send_and_receive() */
int fast_ok_list[MAX_GATHERS];
check_fast_ok(dir) int dir; {
register int n_send_msgs, n_recv_msgs;
register comlink *compt;
/* Find out if every node has one send and one receive message.
	If so, no ordering is needed. */
for(n_recv_msgs=0, compt= neighborlist[dir]; compt != NULL;
	n_recv_msgs++) compt = compt->nextcomlink;
for(n_send_msgs=0, compt= neighborlist_send[dir];
	compt != NULL; n_send_msgs++) compt = compt->nextcomlink;
if( n_send_msgs==1 && n_recv_msgs==1 )fast_ok_list[dir] = 1;
else fast_ok_list[dir]=0;
fast_ok_list[dir] = CMMD_reduce_int(fast_ok_list[dir],CMMD_combiner_and);
}
...
/*TEMPORARY*/
sort_eight_special( fast_ok_list );
}
...
/* start_gather() returns a pointer to a list of msg_tag's, which will
   ...
   on the CM5 we don't worry about the message types yet
   Use msg_id = 1 for message not done, 0 for done
*/
...
int phase,subphase,mask,othernode,msg_size;
int fast_ok;
	...
	mbuf[i].msg_buf=tpt;
	mbuf[i].msg_id = 1;	/* on CM this indicates not sent */
    ...
    if( fast_ok_list[index]==1 ){ /* send and receive one message */
	CMMD_send_and_receive(
	    mbuf[0].msg_node,	/* node from which we get */
	    CM_GATHER_TYPE,
	    mbuf[0].msg_buf,	/* buffer for incoming message */
	    mbuf[0].msg_size,	/* size of incoming message */
	    mbuf[2].msg_node,	/* where we send */
	    CM_GATHER_TYPE,
	    mbuf[2].msg_buf,	/* outgoing message */
	    mbuf[2].msg_size	/* size of outgoing message */
	);
	mbuf[0].msg_id = mbuf[2].msg_id = 0; /* mark them done */
    }
    else{  /* Do it the hard way */
	/* loop over phases. phase = 2^n, where n is the bit number. */
	for(phase=1;phase<numnodes();phase<<=1){
	    mask = phase-1; /* one in all completed bits */

	    for(subphase=0;subphase<2;subphase++){
		/* send if subphase==0 and the bit in my node number
		   is 0, or if both are 1 */
		if( (subphase==0 && (phase&this_node)==0 )
		    || (subphase==1 && (phase&this_node)!=0) ){

		    /* send to other nodes, mark send as completed */
		    /* Use mbuf[i].msg_id = 0 to indicate completed */
		    for(i=n_recv_msgs+1; i<n_recv_msgs+n_send_msgs+1; i++){

			if( mbuf[i].msg_id=0 )continue;
			/* this message has already been sent */
			/* just for efficiency - next loop would
			   exclude these anyway */

			/* if this message is in this phase */
			if( ((phase&this_node) != (phase&mbuf[i].msg_node)) &&
			    ((mask&this_node)==(mask&mbuf[i].msg_node)) ){
			    CMMD_send(mbuf[i].msg_node, CM_GATHER_TYPE,
		    ...
		    /* find out how many requests expected in this phase */
		    j = 0;	/* number of messages left */
		    ...
		    while( j>0 ){
			/* wait for message */
			while( CMMD_msg_pending(CMMD_ANY_NODE,CM_GATHER_TYPE) == 0 );
			/* find sender */
			othernode = CMMD_msg_sender();
			/* find buffer for this message */
		        for(i=0; mbuf[i].msg_buf != NULL; i++){
			    if(mbuf[i].msg_node==othernode)break;
			}
/* CHECK FOR FOULUPS */
if( i==n_recv_msgs )printf("NODE %d BOTCH, UNEXPECTED MESSAGE FROM %d\n",
this_node,othernode);
			/* Accept the message */
			CMMD_receive(othernode,CM_GATHER_TYPE,
			    mbuf[i].msg_buf,mbuf[i].msg_size);
			/* Mark it done */
			mbuf[i].msg_id = 0;
			j--;
msg_size = CMMD_bytes_received();
if( mbuf[i].msg_size != msg_size )
printf("NODE %d BOTCH, WRONG SIZE, expect %d got %d from node %d\n",this_node,
mbuf[i].msg_size,msg_size,mbuf[i].msg_node);
		    }
		}
	    }
        g_sync();	/* Messages sent too early confuse things */
	}
/**g_sync();**/ /* WHY IS THIS NECESSARY? */
    }
    /* return */
    return(mbuf);
}
...
int fast_ok;
    ...
    for(i=n_recv_msgs+1, compt= neighborlist_send[index]; compt != NULL;
	i++,compt = compt->nextcomlink){
	/* Use same buffers to gather data. Remember that when we gather
	   on even sites we must send odd sites, etc. Since the receiving
	   node lists its even sites first, if we are gathering all
	   sites we must do our odd sites first.  */
        switch (parity){
    ...
    /* for each node which has neighbors of my sites */
    for(i=0; i<n_recv_msgs; i++){ mbuf[i].msg_id = 1; }
...
register int i;
    /* Nothing to do yet on CM; don't have asynchronous communications */
    if(mbuf==NULL)return;
    return;
}
...
register int i,i0;
    ...
    for(i=i0; mbuf[i].msg_buf != NULL; i++){
        /* In CM5, messages were synchronous so don't wait */
...
char *tbuf;		/* temporary buffer pointer */
int n_send_msgs, n_recv_msgs;
int fast_ok,phase,subphase,mask,msg_size;
	...
	if( othernode==this_node ){
	    dest[i] =
		F_PT( &lattice[node_index(tx,ty,tz,tt)], field );
	}
	else{
	...
	if( othernode != this_node ){
	    ...
	    *(int *)tpt = node_index(tx,ty,tz,tt);
	    memcpy( tpt+sizeof(int), F_PT(s,field), size);
	    to_nodes[j].count++;
	}
    }
    ...
    /* Find out if every node has one send and one receive message.
	If so, no ordering is needed. */
    if( n_send_msgs==1 && n_recv_msgs==1 )fast_ok = 1;
    else fast_ok=0;
    fast_ok = CMMD_reduce_int(fast_ok,CMMD_combiner_and);

    if( fast_ok==1 ){ /* send and receive one message */
	CMMD_send_and_receive(
	    mbuf[0].msg_node,	/* node from which we get */
	    CM_GATHER_TYPE,
	    mbuf[0].msg_buf,	/* buffer for incoming message */
	...
	for(phase=1;phase<numnodes();phase<<=1){
	    ...
	    for(subphase=0;subphase<2;subphase++){
/**if(this_node==0)printf("PHASE %d %d\n",phase,subphase);**/
		/* send if subphase==0 and the bit in my node number
		   is 0, or if both are 1 */
		if( (subphase==0 && (phase&this_node)==0 )
		    || (subphase==1 && (phase&this_node)!=0) ){
			...
			if( ((phase&this_node) != (phase&mbuf[i].msg_node)) &&
				...
				mbuf[i].msg_buf,mbuf[i].msg_size);
			    mbuf[i].msg_id = 0;
/**if(mbuf[i].msg_node==0)printf("node %d to node 0, phase = %d, firstindex %d\n",
this_node,phase, *(int *)mbuf[i].msg_buf );**/
			}
		    }
		}
		else{ /* I receive */
...
/**if(this_node==0)printf("PHSE %d J %d\n",phase,j);**/
		    /* receive messages until done */
		    while( j>0 ){
			/* wait for message */
			while( CMMD_msg_pending(CMMD_ANY_NODE,CM_GATHER_TYPE) == 0 );
...
this_node,othernode);
			...
			CMMD_receive(othernode,CM_GATHER_TYPE,
			    mbuf[i].msg_buf,mbuf[i].msg_size);
/**if(this_node==0)printf("node 0 from node %d, phase = %d, firstindex %d\n",
mbuf[i].msg_node,phase, *(int *)mbuf[i].msg_buf );**/
			/* Mark it done */
			mbuf[i].msg_id = 0;
			j--;
msg_size = CMMD_bytes_received();
...
/**if(this_node==0)printf("START_GENERAL: sent and received\n");**/

    /* for each node which has neighbors of my sites */
    for(i=0; i<n_recv_msgs; i++){
	/* set pointers in sites to correct location */
/* TEMPORARY PATCH.  Need nicer way to control loop */
/**
nsites = mbuf[i].msg_size/tsize;
	for(j=0;j<nsites;j++){
	    k = *(int *)( mbuf[i].msg_buf + j*tsize );
	    dest[k] = mbuf[i].msg_buf + j*tsize + sizeof(int);
	}
**/
	for(j=0;j<mbuf[i].msg_size;j+=tsize){
	    /* k = index of site on this node, sent in message */
	    k = *(int *)( mbuf[i].msg_buf + j );
	    dest[k] = mbuf[i].msg_buf + j + sizeof(int);
	}
    }
/**if(this_node==0)printf("START_GENERAL: set pointers\n");**/

    /* free temporary arrays */
    if( n_recv_msgs != 0)free(from_nodes);
...
register int tonode;
    for(tonode=0;tonode<numnodes();tonode++)if(tonode!=mynode()){
	CMMD_send( tonode, PARAM_TYPE, buf, sizeof(params) );
    }
}
get_parameters(buf) params *buf; {
    CMMD_receive( 0, PARAM_TYPE, buf, sizeof(params) );
}

/* SEND AND RECEIVE FIELD */
send_field(buf,size,tonode) char *buf; int size,tonode; {
    CMMD_send(tonode,FIELD_TYPE,buf,size);
}
send_field_to_host(buf,size,tonode) char *buf; int size,tonode; {
    CMMD_send(CMMD_host_node(),FIELD_TYPE,buf,size);
}
get_field(buf,size) params *buf; int size; {
    CMMD_receive(CMMD_ANY_NODE,FIELD_TYPE,buf,size);
}

/* BASIC COMMUNICATIONS FUNCTIONS */

/* Tell what kind of machine we are on */
static char name[]="CM5";
char * machine_type(){
    return(name);
}
...
void send_integer(tonode,address) int tonode; int *address; {
    CMMD_send(tonode,SEND_INTEGER_TYPE,address,sizeof(int));
}
...
int zero,type;
    CMMD_receive(CMMD_ANY_NODE,SEND_INTEGER_TYPE,address,sizeof(int));
}

int timer_started = 0;	/* remember if we started a timer */
...
double x;
    ...
    x = CMMD_node_timer_elapsed(0);
    /**x = CMMD_node_timer_busy(0);**/
    CMMD_node_timer_start(0);
    return( x );
    /* I don't know if we want elapsed or busy time  DT 2/18/92 */
}

/* version of exit for multinode processes -- kill all nodes */
terminate(status) int status; {
int i;
    CMMD_error("Termination: node %d, status = %d\n",this_node,status);
}
