...
complex ploop_staple(alpha_fuzz)float alpha_fuzz; {
  register int i,j,t;
  register site *st;
  msg_tag *tag;
  complex sum;
  ...
  for(dir=XUP;dir<=TUP;dir++)if(dir != TUP){
    ...
    FORALLSITES(i,st){
      ...
      mult_su3_nn( &tmat1, gen_pt[0][i], &(st->tempmat1) );
    }
    ...
    if(start){	/* this is the first contribution to staple */
      FORALLSITES(i,st){
	mult_su3_nn( &(st->link[dir]), gen_pt[2][i], &tmat1);
	mult_su3_na( &tmat1, gen_pt[0][i], &(st->staple) );
      }
      start=FALSE;
    }
    else{
      FORALLSITES(i,st){
	mult_su3_nn( &(st->link[dir]), gen_pt[2][i], &tmat1);
	mult_su3_na( &tmat1, gen_pt[0][i], &tmat2 );
    ...
    wait_gather(tag1);
    FORALLSITES(i,st){
      add_su3_matrix( &(st->staple),gen_pt[1][i],&(st->staple));
    }
    cleanup_gather(tag0);
    cleanup_gather(tag1);
    cleanup_gather(tag2);
  ...
  wait_gather(tag);
  FOREVENSITES(i,st){
    mult_su3_nn( &(st->staple), gen_pt[0][i], &(st->tempmat1));
  }
  cleanup_gather(tag);
  ...
  FOREVENSITES(i,st){
    if( st->t > 1 )continue;
    plp_staple = trace_su3( &(st->tempmat1) );
    CSUM(sum,plp_staple);
  }
  g_complexsum( &sum );
  plp_staple.real = oma2nt*sum.real /((float)(nx*ny*nz));
  plp_staple.imag = oma2nt*sum.imag /((float)(nx*ny*nz));
  return(plp_staple);
}
