...
/*
   ...
   Intel Paragon version
    Sunmos version - get rid of hrecv and killcube 
    Use beta < 0 to stop?
...
int mask,sticky;
if( mynode()==0 )printf("WARNING: underflow traps not disabled\n");
    /* Turn off underflow traps later  */
    /**
    if( mynode()==0 )printf("Mode = %d, Mask = %d,  Sticky = %d\n",
	fpgetround(),fpgetmask(),fpgetsticky() );
    mask = fpgetmask;
    sticky = fpgetsticky;
    fpsetround( FP_RZ );
    if( mynode()==0 )printf("Mode = %d, Mask = %d,  Sticky = %d\n",
    ...
    **/
    /**
    printf(
	"START: node %d, nsend %d nrecv %d msgout %d msgin %d nri %d nrb %d\n",
        this_node, nxinfo.nsend, nxinfo.nrecv, nxinfo.msgout,
        nxinfo.msgin, nxinfo.nrimm, nxinfo.nrbuf );
    printf("START: node %d, align %d over %d under %d inex %d sou %d\n",
        this_node, nxinfo.align_traps, nxinfo.fpo_traps, nxinfo.fpu_traps,
        nxinfo.fpi_traps, nxinfo.fps_traps);
    **/
}
/** To use the nxinfo information, at end of code put:
printf("END: node %d, nsend %d nrecv %d msgout %d msgin %d nri %d nrb %d\n",
this_node, nxinfo.nsend, nxinfo.nrecv, nxinfo.msgout,
nxinfo.msgin, nxinfo.nrimm, nxinfo.nrbuf );
printf("END: node %d, align %d over %d under %d inex %d sou %d\n",
this_node, nxinfo.align_traps, nxinfo.fpo_traps, nxinfo.fpu_traps,
nxinfo.fpi_traps, nxinfo.fps_traps);
**/
