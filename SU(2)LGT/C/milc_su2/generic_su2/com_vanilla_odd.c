...
/* start_gather() returns a pointer to a list of msg_tag's, which will
   ...
   example:
	msg_tag *tag;
	tag = start_gather( F_OFFSET(phi), sizeof(su2_vector), XUP,
	    EVEN, gen_pt[0] );
	  ** do other stuff **
	wait_gather(tag);
	...
	restart_gather( F_OFFSET(phi), sizeof(su2_vector), XUP,
	    EVEN, gen_pt[0], tag );
	  ** do other stuff **
	wait_gather(tag);
...
int size;		/* size in bytes of the field (eg sizeof(su2_vector))*/
...
/* start_general_gather() returns a pointer to a list of msg_tag's, which will
   ...
   example:
	...
	tag = start_general_gather( F_OFFSET(phi), sizeof(su2_vector), disp,
	    EVEN, gen_pt[0] );
	  ** do other stuff **
...
/* Return a pointer to a field in the lattice at some coordinates.
   ...
   Example:  
	su2_matrix *pt;
	pt = (su2_matrix *)field_pointer_at_coordinates(
	    F_OFFSET(xlink), sizeof(su2_matrix), x,y,z,t );
	... do stuff here ...
